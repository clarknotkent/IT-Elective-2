...
| Module | Author | Original submission |
|---|---|---|
| Inventory Management (main inventory, transfer, PDF report) | James Douglas Ancheta & Kent Elrond Andionne Aspa | `inventoryManagement/` |
| Ingredient Catalog | Angel Gabriel Litob | `Ingredient List/PyFinalProj/` |
| Stock In / Stock Out | Trisha Ryle Pusta | `Stock-in_Stock-out/` |
| Deployment (PythonAnywhere) | Chris Maynard Ampon | — |
| Unified schema, tests, docs | Kent Elrond Andionne Aspa | this repo |
...
Its `.git` history shows it starts from a public template,
...
`stock_movements` table (see `models.py`) instead, with
`InventoryItem.quantity` derived from them rather than held in memory.
...
Each contributor's own submission is kept, code and screenshots only (no
virtual environments or databases), under `original-submissions/` — see
`original-submissions/README.md`.

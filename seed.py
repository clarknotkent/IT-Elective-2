...
# Fixed reference point so the seeded log reads the same on every run.
SEED_DAY = datetime(2024, 12, 13, 9, 0, tzinfo=timezone.utc)
...
def seed():
    ...
    with app.app_context():
        ...
        for name, itype in INGREDIENTS:
            ...
            if existing:
                ...
                ing = Ingredient(
                    ...
                    created_at=SEED_DAY - timedelta(days=7),
                )
                db.session.add(ing)
                db.session.flush()
        ...
        if PreparationInventory.query.count() == 0:
            transfers = [
                ("Vodka", 5, date(2024, 12, 20)),
                ("Cola", 10, date(2024, 12, 20)),
            ]
                ...
                if item:
                    db.session.add(
                        PreparationInventory(
                            ...
                            quantity=qty,
                            booking_date=bdate,
                            transferred_at=SEED_DAY - timedelta(days=1),
                        )
                    )
